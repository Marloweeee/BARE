...
######
import math
from .beit3_modal_bridge import VBC
    ...
    def __init__(self, args, depth, is_moe_layer=False, is_encoder_decoder=False):
        ...
        if args.deepnorm:
            if is_encoder_decoder:
                self.alpha = (
                    math.pow(
                        math.pow(args.encoder_layers, 4) *
                        args.decoder_layers, 0.0625
                    )
                    * 0.81
                )
            else:
    ...
    def forward_embedding(
        ...
        if self.embed_positions is not None:
            if src_tokens is not None:
                x = embed + \
                    self.embed_positions(src_tokens, positions=positions)
            else:
...
class EncoderModified(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.layers = nn.ModuleList([])
        # 每隔 step 层插入一个跨模态桥接层（VBC）
        self.step = 4
        start_layer = 0
        totle_layers=args.encoder_layers
        self.modal_bridge_layers = nn.ModuleList([])
        self.modal_bridge_layer_indices = list(range(start_layer, totle_layers, int(self.step)))
        ...
        # 按照预定义的 indices 构建对应数量的 VBC 模块
        for _ in self.modal_bridge_layer_indices:
            self.modal_bridge_layers.append(
                VBC(embed_dim, 8)
            )
    ...
    def forward(
        ...
        **kwargs
        ...
        # incremental_state is not None during inference if we use the bidirectional encoder as a generator as in s2s-ft (https://arxiv.org/abs/2110.13640)
        l_aux = []
        # 级联残差：由前一个 VBC 的输出传递给下一个 VBC，初始为 None（等价于 0）
        vbc_residual = None
        for idx, layer in enumerate(self.layers):
            x, l_aux_i = layer(
                x,
            ...
            # 在指定层插入 VBC，并将上一轮的残差显式传入，再显式接收新的残差
            if idx in self.modal_bridge_layer_indices:
                layer_idx = self.modal_bridge_layer_indices.index(idx)
                x, vbc_residual = self.modal_bridge_layers[layer_idx](
                    x, prev_output_feat=vbc_residual
                )
...
class BEiT3(nn.Module):
    def __init__(self, args, **kwargs):
        ...
        embed_positions = MutliwayEmbedding(
            modules=[
                PositionalEmbedding(
                    self.vision_embed.num_position_embeddings() + 2, args.encoder_embed_dim),
                PositionalEmbedding(args.max_source_positions,
                                    args.encoder_embed_dim),
            ],
            dim=1,
        )
        self.encoder = EncoderModified(
            args,
            embed_tokens=None,
