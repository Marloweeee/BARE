...
def get_args_parser():
    parser = argparse.ArgumentParser('BARE Training Args', add_help=False)
    ...
    parser.add_argument('--lr_exponential', default=0.9,
                        type=float, help='lr exponential')
    parser.add_argument('--clip_max_norm', default=0.,
    ...
    # Model parameters
    parser.add_argument('--model_name', type=str, default='BARE',
                        help="Name of model to be exploited.")
    parser.add_argument('--extract_layer', default=0, type=int)
    parser.add_argument('--warmup', action='store_true',
                        help="If true, vision adapt layer is null")
    ...
    parser.add_argument('--vit_type', default='base',
                        type=str, help='default ViT model type')

    # gate parameters
    parser.add_argument('--lambda_debias', default=0.1,
                        type=float, help='lambda_debias')
    parser.add_argument('--alpha_debias', default=0.5,
                        type=float, help='alpha_debias')
    parser.add_argument('--learnable_alpha', default=True,
                        type=bool, help='learnable_alpha')
    # drop_path_rate=0.1,
    parser.add_argument('--drop_path_rate', default=0.1,
                        type=float, help='drop path rate')
    parser.add_argument('--vocab_size', default=64010,
                        type=int, help='vocabulary size')

    parser.add_argument('--norm_layer', default='LayerNorm',
                        type=str, help='default layer norm type')
    parser.add_argument('--freeze_layer', default=-1,
                        type=int, help='freeze layer number')
    parser.add_argument('--vision_embed_proj_interpolate', default=False,
                        type=bool, help='vision embed projection interpolate')
    parser.add_argument('--spm_model', default='./weights/beit3.spm',
                        type=str, help='pretrain model path')

    # lora parameters
    # parser.add_argument('open_lora_parameter_update',default=True, type=bool, help='open lora parameter update')
    ...
    parser.add_argument('--dataset', default='referit',
                        type=str, help='referit/unc/unc+/gref/gref_umd')
    ...
    parser.add_argument('--use_cot_prompt', action='store_true',
                        help="If true, using COT prompt")
    parser.add_argument('--cot_length', type=int,
                        default=0, help="Prompt template")
    parser.add_argument('--use_contrastive_loss',
                        action='store_true', help="If true, use contrastive loss")
    parser.add_argument('--use_rtcc_constrain_loss',
                        action='store_true', help="If true, use contrastive loss")
    parser.add_argument('--use_mask_loss', action='store_true',
                        help="If true, use segmentation loss")
    ...
    parser.add_argument('--retrain', default='',
                        help='retrain from checkpoint')
    parser.add_argument('--adapt_mlp', action='store_true',
                        help="If true, use contrastive loss")
    parser.add_argument('--normalize_before', action='store_true',
                        help="If true, use normalize_before")
    parser.add_argument('--save_hilora_clip', action='store_true',
                        help="If true, save hilora clip model")
    parser.add_argument('--hi_lora_stage', default=0,
                        type=int, help='lora stage')
    parser.add_argument('--hi_lora_retrain', default='',
                        help='lora retrain from checkpoint')
    parser.add_argument('--hi_lora_clip', default='',
                        type=str, help='clip model')
    parser.add_argument('--mixup_pretrain', action='store_true',
                        help="If true, use mixup pretraining data")
    parser.add_argument('--enable_adaptive_weights',
                        action='store_true', help="If true, enable adaptive weight")

    # Cross module structure
    parser.add_argument('--cross_num_attention_heads', default=1,
                        type=int, help='cross module attention head number')
    # parser.add_argument('--cross_vis_hidden_size', default=256, type=int, help='cross module hidden size')
    parser.add_argument('--cross_vis_hidden_size', default=512,
                        type=int, help='cross module hidden size')
    # parser.add_argument('--cross_text_hidden_size', default=768, type=int, help='cross module hidden size')
    parser.add_argument('--cross_text_hidden_size', default=512,
                        type=int, help='cross module hidden size')
    parser.add_argument('--cross_hidden_dropout_prob', default=0.1,
                        type=float, help='cross module hidden dropout probability')
    parser.add_argument(
        '--cross_attention_probs_dropout_prob', default=0.1, type=float)
    ...
    parser.add_argument('--beit_model', default='./weights/beit3_base_indomain_patch16_224.pth',
                        type=str, help='BEiT 3 model')
...
def main(args):
    ...
    np.random.seed(seed)
    random.seed(seed)
    print('### INFO ### torch.backends.cudnn.benchmark = {}'.format(
    ...
    close_beit_param_update = False
    if close_beit_param_update:
        for name, param in model.beit3.named_parameters():
            param.requires_grad_(False)

    # p.numel(): number of elements in tensor, using to calculate the number of parameters
    # torch.randn(3,3).numel()=9,torch.randn(3,5,10).numel()=150
    n_parameters_grad = sum(p.numel()
    ...
    if args.lr_scheduler == 'poly':
        ...
        def lr_func(epoch): return args.lr_exponential ** epoch
        lr_scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lr_func)
    else:
    ...
    # if args.beit_model != "":
    #     checkpoint = torch.load(args.beit_model, map_location='cpu')
    #     print("\nmodel structures: \n", model_without_ddp.beit3)
    #     missing_keys, unexpected_keys = model_without_ddp.beit3.load_state_dict(checkpoint['model'], strict=False)
    #     print('Missing keys when loading lora fine-tuned beit model:')
    #     print(missing_keys)
    #     print('Unexpected additional keys when loading lora fine-tuned beit model:')
    ...
    if args.hi_lora_stage and not args.resume:
        ...
        print("hi_lora_stage, load beit3 model")
        checkpoint = torch.load(args.hi_lora_clip, map_location='cpu')
        missing_keys, unexpected_keys = model_without_ddp.beit3.load_state_dict(
            checkpoint['model'], strict=False)
        ...
        beit_checkpoint_path = os.path.join(
            args.output_dir, "beit3_lora_stage.pth")
        utils.save_on_master(
            {"model": model_without_ddp.beit3.state_dict()}, beit_checkpoint_path)
        print("HiLoRA BEIT3 checkpoint saved!")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir:
            ...
            if val_stats['accu'] > best_accu:
                checkpoint_paths.append(os.path.join(
                    args.output_dir, 'best_checkpoint.pth'))
                best_accu = val_stats['accu']
                if args.save_hilora_clip:
                    clip_checkpoint_path = os.path.join(
                        args.output_dir, "clip_lora_stage_with_bridge.pth")
                    utils.save_on_master(
                        {"model": model_without_ddp.clip.state_dict()}, clip_checkpoint_path)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        'BARE training script', parents=[get_args_parser()])
