...
def build_target(args, gt_bbox, pred, device):
    ...
    for ii in range(batch_size):
        ...
        for scale_ii in range(num_scales):
            ...
            gt_box = torch.from_numpy(
                np.array([0, 0, gw.cpu().numpy(), gh.cpu().numpy()])).float().unsqueeze(0)
...
def yolo_loss(pred_list, target, gi, gj, best_n_list, device, w_coord=5., w_neg=1. / 5, size_average=True):
    ...
    for ii in range(batch_size):
        ...
        pred_bbox[ii, 2:4] = pred_list[best_n_list[ii] //
                                       3][ii, best_n_list[ii] % 3, 2:4, gj[ii], gi[ii]]
    ...
    for scale_ii in range(num_scale):
        pred_conf_list.append(
            pred_list[scale_ii][:, :, 4, :, :].contiguous().view(batch_size, -1))
        gt_conf_list.append(
            target[scale_ii][:, :, 4, :, :].contiguous().view(batch_size, -1))
...
def trans_vg_loss(args, batch_pred, batch_target, tgt_mask, text_eos, img_cls=None, visu_sim=None, seg_mask=None):
    ...
    if args.use_rtcc_constrain_loss:
        ...
        obj_mask = mdetr_interpolate(
            tgt_mask.float(), (patch_num, patch_num), mode="nearest")[:, 0] > 0.5
        obj_mask = obj_mask.flatten(1).float()
        visu_sim = visu_sim.flatten(1)
        losses['loss_algin_focal'] = sigmoid_focal_loss(
            visu_sim, obj_mask, num_boxes) * coef_focal
        losses['loss_align_dice'] = dice_loss(
            visu_sim, obj_mask, num_boxes) * coef_dice

    if args.use_mask_loss:
        coef_focal = 20.0
        coef_dice = 2.0
        src_mask = mdetr_interpolate(
            seg_mask, size=tgt_mask.shape[-2:], mode="bilinear", align_corners=False)
        src_mask = src_mask.flatten(1)
        tgt_mask = tgt_mask.flatten(1).float()

        losses['loss_seg_focal'] = sigmoid_focal_loss(
            src_mask, tgt_mask, num_boxes) * coef_focal
        losses['loss_seg_dice'] = dice_loss(
            src_mask, tgt_mask, num_boxes) * coef_dice

    return losses


def bare_loss(args, batch_pred, batch_target, seg_mask, tgt_mask, logits_per_img, visu_sim):
    batch_size = batch_pred.shape[0]
    # world_size = get_world_size()
    num_boxes = batch_size
    ...
    if args.use_contrastive_loss:
        loss_contrastive = clip_loss(logits_per_img)
        losses['loss_contrastive'] = loss_contrastive
