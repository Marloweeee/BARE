...
def convert_examples_to_features(examples, seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # 1 for valid tokens and 0 for padding tokens.
...
def load_expression_tokenize_beit3(example,max_query_len,tokenizer):
    """
    ...
    if len(tokens)==0:
        raise ValueError("The length of tokens is 0, please check the input text.")
    
    if len(tokens) > max_query_len-2:
        tokens = tokens[:max_query_len-2]

    
    tokens=[tokenizer.bos_token_id]+tokens+[tokenizer.eos_token_id]
    token_len = len(tokens)
    padding_mask=[0]*token_len+[1]*(max_query_len-token_len)
    ref_exp_idx=tokens+[tokenizer.pad_token_id]*(max_query_len-token_len)

    word_id=np.array(ref_exp_idx,dtype=int)
    word_mask=np.array(padding_mask,dtype=int)
...
class TransVGDataset(data.Dataset):
    ...
    def __init__(self, args, data_root, split_root='data', dataset='referit',
        ...
        self.tokenizer=XLMRobertaTokenizer(args.spm_model)
    ...
    def __getitem__(self, idx):
        ...
        if self.lstm:
            ...
            word_id, word_mask = load_expression_tokenize_beit3(phrase, self.query_len, self.tokenizer)
