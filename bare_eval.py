...
def get_args_parser():
    parser = argparse.ArgumentParser('BARE Evaluate args', add_help=False)
    ...
    parser.add_argument('--pretrain', default='/mnt/sdb/lihb/codes/HiVG/OneTower-HiVG/HiVG-BEIT3/pretrained_checkpoints/beit3_base_patch16_224.pth',
    ...
    parser.add_argument('--dataset', default='referit',
                        type=str, help='referit/flickr/unc/unc+/gref')
    ...
    parser.add_argument('--eval_set', default='test',
                        type=str)  # 'testA', 'testB', 'val'
    parser.add_argument('--eval_model', default='', type=str)

    parser.add_argument('--prompt', type=str,
                        default='{pseudo_query}', help="Prompt template")
...
def main(args):
    ...
    if args.output_dir and utils.is_main_process():
        with (output_dir / "eval_{}_{}.txt".format(args.dataset, args.eval_set)).open("a") as f:
            f.write(str(args) + "\n")
            f.flush()
    ...
    if args.save_hilora_clip:
        beit3_checkpoint_path = os.path.join(
            args.output_dir, "beit3_lora_stage.pth")
        torch.save({"model": model_without_ddp.beit3.state_dict()},
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        'BARE evaluation script', parents=[get_args_parser()])
