#!/bin/bash
CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 --master_port 26887 --use_env bare_train.py \
    --num_workers 16 --epochs 60 --batch_size 32 --lr 0.0001   \
    --data_root ./ln_data --split_root ./data \
    --vit_type base --vl_hidden_dim 768 --vl_dim_feedforward 3072 \
    --beit_model ./weights/beit3_base_indomain_patch16_224.pth \
    --output_dir ./work_dir/BARE/mixup_base \
    --sup_type full --lr_scheduler cosine --aug_crop --aug_scale --aug_translate --imsize 224 --max_query_len 77 \
    --normalize_before --enable_adaptive_weights --dataset mixup --use_contrastive_loss  --use_rtcc_constrain_loss --use_mask_loss;

infer_pth='./work_dir/BARE/mixup_base/best_checkpoint.pth'
# unc
CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 --master_port 28888 --use_env bare_eval.py \
    ...
    --output_dir ./work_dir/BARE/mixup_base_infer;
CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 --master_port 28888 --use_env bare_eval.py \
    --num_workers 8 --batch_size 64  --dataset unc \
    --vit_type base --vl_hidden_dim 768 --vl_dim_feedforward 3072 \
    --imsize 224 --max_query_len 77 --normalize_before --enable_adaptive_weights --use_mask_loss  \
