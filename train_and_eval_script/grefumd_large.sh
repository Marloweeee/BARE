...
CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 --master_port 28887 --use_env bare_train.py \
    ...
    --output_dir ./work_dir/BARE/grefumd_large \
    --sup_type full --lr_scheduler cosine --aug_crop --aug_scale --aug_translate --imsize 224 --max_query_len 77 \
    --normalize_before --enable_adaptive_weights --dataset gref_umd --use_contrastive_loss  --use_rtcc_constrain_loss --use_mask_loss;
    # --resume ./work_dir/BARE/grefumd_large/checkpoint.pth

# Infer val
CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 --master_port 28888 --use_env bare_eval.py \
    --num_workers 8 --batch_size 64  --dataset gref_umd --vl_hidden_dim 1024 --vl_dim_feedforward 4096 \
    --vit_type large \
    --imsize 224 --max_query_len 77 --normalize_before --enable_adaptive_weights --use_mask_loss  \
    --data_root ./ln_data --split_root ./data \
    --eval_model ./work_dir/BARE/grefumd_large/best_checkpoint.pth --eval_set val \
    --output_dir ./work_dir/BARE/grefumd_large_infer;
...
CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 --master_port 28888 --use_env bare_eval.py \
    ...
    --imsize 224 --max_query_len 77 --normalize_before --enable_adaptive_weights --use_mask_loss  \
    --data_root ./ln_data --split_root ./data \
    --eval_model ./work_dir/BARE/grefumd_large/best_checkpoint.pth --eval_set test \
    --output_dir ./work_dir/BARE/grefumd_large_infer;
